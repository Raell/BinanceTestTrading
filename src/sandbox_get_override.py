...
from cryptofeed.defines import GET, DELETE
...
async def get_account_info(exchange: BinanceRestMixin) -> Dict[str, str]:
    ...
    data = await exchange._request(GET, ACCOUNT, auth=True, api=SANDBOX_REST_API + SANDBOX_REST_ACCOUNT)
    return data

async def cancel_all_orders(exchange: BinanceRestMixin, symbol: str) -> None:
    # Manual implementation of CANCEL ALL for Sandbox API
    await exchange._request(DELETE, ALL_OPEN_ORDER, auth=True, api=SANDBOX_REST_API + SANDBOX_REST_ORDER, payload={'symbol': symbol})